...
exports.DeliveryConfirmationStep = DeliveryConfirmationStep;
function DeliveryConfirmationStep(_a) {
    var orderId = _a.orderId, paymentMethod = _a.paymentMethod, total = _a.total, onClose = _a.onClose;
    //   const { t } = useTranslation();
    var handleDownloadPDF = function () {
        //   const { t } = useTranslation();
        window.open("/api/orders/".concat(orderId, "/receipt?lang=ro"), '_blank');
    };
    return (<div className="text-center space-y-6">
      ...
      <div className="space-y-3 pt-4">
        <button onClick={handleDownloadPDF} className="w-full px-6 py-3 bg-[#FF6B35] text-white rounded-lg font-semibold hover:bg-[#e55a2b] transition-all">
          <i className="fas fa-file-pdf mr-2"></i>"salveaza dovada pdf"</button>
        <button onClick={onClose} className="w-full px-6 py-3 bg-gray-200 text-gray-800 rounded-lg font-semibold hover:bg-gray-300 transition-all">
