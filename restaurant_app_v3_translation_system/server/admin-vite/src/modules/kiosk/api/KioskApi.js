...
export const kioskLogin = async (username, password) => {
  try {
    ...
    console.log('📤 kioskLogin - Trimitere date:', loginData);
    
    const response = await httpClient.post('/api/admin/auth/login', loginData);
    ...
    if (error.response) {
      ...
      const errorMessage = errorData.error || errorData.message || errorData.msg || 'Eroare la autentificare';
      console.error('❌ kioskLogin - Eroare server - FULL DATA:', errorData);
      console.error('❌ kioskLogin - Eroare server:', {
      ...
      if (error.response.status === 400) {
        console.error('❌ kioskLogin - 400 Bad Request - checking for details...');
        console.error('❌ kioskLogin - errorData keys:', Object.keys(errorData));
        console.error('❌ kioskLogin - errorData.errors:', errorData.errors);
        console.error('❌ kioskLogin - errorData.details:', errorData.details);
        console.error('❌ kioskLogin - Full errorData:', JSON.stringify(errorData, null, 2));
        
        if (errorData.errors) {
          throw new Error(`Eroare validare: ${JSON.stringify(errorData.errors)}`);
        }
      }
...
export const getTablesStatus = async () => {
  try {
    ...
    // API-ul returnează { success: true, tables: [] }
    ...
    console.error('❌ Eroare la obținerea statusului meselor:', error);
...
// Fiscalizare
export const fiscalizeOrder = async (orderId, payment = {}) => {
...
// Rapoarte ospătar (doar admin) - folosește endpoint-ul REAL /api/reports/staff-live
export const getStaffLiveReport = async (waiterId, shiftDate) => {
  const response = await httpClient.get('/api/reports/staff-live', {
    params: {
      waiter_id: waiterId,
