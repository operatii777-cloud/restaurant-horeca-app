  ...
  return (
    <KioskThemeProvider>
      <KioskLoginModalProvider>
        <KioskMainLayoutInner session={session} onLogout={handleLogout} sidebarCollapsed={sidebarCollapsed} />
      </KioskLoginModalProvider>
    </KioskThemeProvider>
  );
};

// Component intern care folosește contextul pentru a accesa showLoginModal
const KioskMainLayoutInner = ({ session, onLogout, sidebarCollapsed }) => {
  const { showLoginModal, closeLoginModal, onLoginSuccess } = useKioskLoginModal();

  const handleLoginSuccess = (newSession) => {
    console.log('✅ KioskMainLayout - Login success, sesiune:', newSession);
    if (onLoginSuccess) {
      onLoginSuccess(newSession);
    }
    // Reîncarcă pagina pentru a actualiza sesiunea
    window.location.reload();
  };

  console.log('🔍 KioskMainLayoutInner render - showLoginModal:', showLoginModal);

  return (
    <>
      <div className={`kiosk-main-layout kiosk-layout ${sidebarCollapsed ? 'kiosk-main-layout--sidebar-collapsed' : ''}`}>
        <KioskSidebar user={session} onLogout={onLogout} />
        <div className="kiosk-main-layout__content">
          <Outlet />
        </div>
      </div>
      
      {/* 🔴 FIX: Renderizează KioskLoginModal pentru a se afișa când showLoginModal este true */}
      <KioskLoginModal
        show={showLoginModal}
        onHide={closeLoginModal}
        onLoginSuccess={handleLoginSuccess}
        title="Autentificare KIOSK"
      />
    </>
  );
};
