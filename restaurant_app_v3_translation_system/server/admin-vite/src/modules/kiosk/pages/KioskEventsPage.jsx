...
export const KioskEventsPage = () => {
  ...
  const loadEvents = async () => {
    ...
    try {
      ...
      // CRITICAL: Verifică că res.data este un array înainte de a-l seta
      if (res.data && Array.isArray(res.data)) {
        setEvents(res.data);
      } else if (res.data) {
        // Dacă res.data există dar nu e array, încearcă să extragă array-ul
        const eventsArray = res.data.events || res.data.data || [];
  ...
  const KanbanColumn = ({ status, title, icon }) => {
    // CRITICAL: Verifică că events este un array înainte de a folosi filter
    const items = Array.isArray(events) ? events.filter(e => e && e.status === status) : [];
    return (
      <div style={{
