  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (response.data?.success && Array.isArray(response.data.data)) {
        // Normalizează datele pentru a preveni erori cu valori undefined/null
        const products: TopProduct[] = response.data.data.map((p: any) => ({
          id: p.id || 0,
        ...
        const totalRevenue = products.reduce((sum: number, p: TopProduct) => sum + (p.total_revenue || 0), 0);
        const totalOrders = products.reduce((sum: number, p: TopProduct) => sum + (p.times_ordered || 0), 0);
            ...
            <Col md={3}>
              ...
              <div>
                <Button variant="primary" onClick={loadData}>
                  <i className="fas fa-sync me-2"></i>Actualizează</Button>
              </div>
            </Col>
          </Row>
        </Card.Body>
                    ...
                    <tbody>
                      {topProductsByRevenue.map((product, index) => (
                        <tr key={product.id}>
                          ...
                          <td>
                            {product.profit_margin_percent !== undefined && product.profit_margin_percent !== null ? (
