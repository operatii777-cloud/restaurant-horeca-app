...
exports.mapRawOrderToCanonical = mapRawOrderToCanonical;
var BAR_CATEGORIES = [
...
function isBarCategory(category) {
    if (!category)
        return false;
    return BAR_CATEGORIES.some(function (bc) { return category.includes(bc); }) ||
 ...
 */
function mapRawOrderToCanonical(raw) {
    ...
    if (raw.type === 'delivery')
        type = 'Delivery';
    else if (raw.type === 'takeout')
        type = 'takeout';
    else if (raw.type === 'drive_thru')
        type = 'drive_thru';
    else if (raw.type === 'here')
        type = 'dine_in';
    // Map source
    var source = 'POS';
    if (raw.platform === 'KIOSK' || raw.order_source === 'KIOSK_SELF_SERVICE')
        source = 'KIOSK';
    else if (raw.platform === 'MOBILE_APP')
        source = 'QR';
    else if (raw.order_source === 'DELIVERY' || raw.platform === 'delivery')
        source = 'DELIVERY';
    else if (raw.platform === 'pos')
        source = 'POS';
    return {
        id: raw.id,
        code: raw.code || raw.order_number || "#".concat(raw.id),
        status: status,
        type: type,
        source: source,
