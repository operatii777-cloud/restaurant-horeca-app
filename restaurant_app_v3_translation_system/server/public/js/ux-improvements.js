...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Wrap console.log pentru a filtra log-urile excesive
    const originalLog = console.log;
    const logHistory = new Map(); // Cache pentru a preveni log-uri duplicate
    const LOG_THROTTLE_MS = 2000; // Throttle pentru mesaje similare
    
    console.log = function(...args) {
        const message = args.join(' ');
        
        // ✅ Filtrează log-urile excesive/debug
        const isDebugLog = message.includes('🔄') || 
                          message.includes('📋') || 
                          message.includes('📡') || 
                          message.includes('📅') || 
                          message.includes('📊') ||
                          message.includes('🌐') ||
                          message.includes('✅ 20 categorii') ||
                          message.includes('✅ Loaded') ||
                          message.includes('✅ Products loaded') ||
                          message.includes('✅ Categories loaded');
        
        if (isDebugLog) {
            // Throttle pentru mesaje similare
            const logKey = message.substring(0, 50); // Primele 50 caractere ca key
            const lastLogTime = logHistory.get(logKey) || 0;
            const now = Date.now();
            
            if (now - lastLogTime < LOG_THROTTLE_MS) {
                return; // Skip log-ul dacă e prea recent
            }
            
            logHistory.set(logKey, now);
            
            // Cleanup cache-ul periodic (păstrează doar ultimele 100 entries)
            if (logHistory.size > 100) {
                const oldestKey = logHistory.keys().next().value;
                logHistory.delete(oldestKey);
            }
        }
