...
class UniversalHorecaBenchmark {
    constructor(configPath = null) {
        ...
        this.cachedData = {}; // For hybrid: masa_id, cod_prod from API
    ...
    loadConfig(configPath) {
        const defaultConfigPath = path.join(__dirname, 'benchmark-config.json');
        if (!configPath && fs.existsSync(defaultConfigPath)) {
    ...
    /**
     * Fetch hybrid-specific data (masa_id, cod_prod) for order creation
     */
    async prepareHybridData() {
        if (this.config.orderPayloadFormat !== 'hybrid') return;
        try {
            const [menuRes, meseRes] = await Promise.all([
                axios.get(`${this.config.baseURL}${this.config.endpoints.menu}`, { timeout: this.config.timeout }),
                axios.get(`${this.config.baseURL}${this.config.endpoints.tables}`, { timeout: this.config.timeout })
            ]);
            const products = this.extractArray(menuRes.data, ['products', 'data', 'menu', 'items']);
            const tables = this.extractArray(meseRes.data, ['tables', 'data']);
            const firstProduct = Array.isArray(products) && products[0] ? products[0] : null;
            const firstTable = Array.isArray(tables) && tables[0] ? tables[0] : null;
            this.cachedData.cod_prod = firstProduct?.cod_prod ?? firstProduct?.id ?? this.config.testData?.cod_prod ?? '1';
            this.cachedData.masa_id = firstTable?.id ?? firstTable?.masa_id ?? this.config.testData?.masa_id ?? 1;
            this.cachedData.ospatar_id = this.config.testData?.ospatar_id ?? '1';
        } catch (e) {
            this.cachedData.cod_prod = this.config.testData?.cod_prod ?? '1';
            this.cachedData.masa_id = this.config.testData?.masa_id ?? 1;
            this.cachedData.ospatar_id = this.config.testData?.ospatar_id ?? '1';
        }
    }

    /**
     * Build order payload based on config format (v3 or hybrid)
     */
    buildOrderPayload(opts = {}) {
        const t = this.config.testData || {};
        if (this.config.orderPayloadFormat === 'hybrid') {
            const cod = this.cachedData?.cod_prod ?? t.cod_prod ?? '1';
            const masa = this.cachedData?.masa_id ?? t.masa_id ?? 1;
            const ospatar = this.cachedData?.ospatar_id ?? t.ospatar_id ?? '1';
            const cant = opts.quantity ?? t.cant ?? 1;
            const pret = opts.price ?? t.pret_unitar ?? 10;
            const valoare = cant * pret;
            return {
                masa_id: masa,
                ospatar_id: ospatar,
                linii: [{ cod_prod: String(cod), cant, pret_unitar: pret, valoare }],
                status: opts.status ?? 'plasata'
            };
        }
        return {
            type: opts.type ?? 'dine_in',
            items: [{
                product_id: t.product_id ?? 1,
                name: opts.name ?? 'Test Product',
                quantity: opts.quantity ?? t.test_quantity ?? 1,
                price: opts.price ?? t.test_price ?? 10
            }],
            total: (opts.quantity ?? t.test_quantity ?? 1) * (opts.price ?? t.test_price ?? 10),
            table: opts.table ?? t.table_number ?? 'T1',
            payment_method: opts.payment_method ?? 'cash',
            platform: opts.platform ?? 'BENCHMARK',
            ...opts
        };
    }
    ...
    async testCoreFunctionality() {
        ...
        await this.runTest(module, 'Order Creation', 20, async () => {
            const start = performance.now();
            const orderData = this.buildOrderPayload();
            ...
            if (![200, 201, 202].includes(response.status)) {
                throw new Error(`Order creation failed with status ${response.status}: ${JSON.stringify(response.data)}`);
            }

            const orderId = response.data?.orderId || response.data?.order_id || response.data?.order?.id || response.data?.id || response.data?.comanda_id;
    ...
    async testPerformance() {
        ...
        await this.runTest(module, 'Order Processing Time', 25, async () => {
            ...
            for (let i = 0; i < orderCount; i++) {
                const start = performance.now();
                try {
                    const orderPayload = this.buildOrderPayload({ table: `BENCH-${i}` });
                    await axios.post(
                        `${this.config.baseURL}${this.config.endpoints.orderCreate}`,
                        orderPayload,
                        { timeout: this.config.timeout, validateStatus: () => true }
    ...
    async testPOSFeatures() {
        ...
        await this.runTest(module, 'Split Bill', 10, async () => {
            const orderData = this.buildOrderPayload({ table: 'SPLIT-TEST' });
            if (this.config.orderPayloadFormat !== 'hybrid') {
                orderData.split_bill = { method: 'equal', parts: 2 };
            }
        ...
        await this.runTest(module, 'Receipt Generation', 10, async () => {
            const orderData = this.buildOrderPayload({ table: 'RECEIPT-TEST' });
            ...
            const orderId = orderResponse.data?.orderId || orderResponse.data?.order_id || orderResponse.data?.order?.id || orderResponse.data?.comanda_id;
        ...
        await this.runTest(module, 'Protocol Payment (Zero Amount)', 10, async () => {
            if (this.config.orderPayloadFormat === 'hybrid') throw new Error('Protocol not supported by hybrid (POS-only feature)');
            const orderData = this.buildOrderPayload({ table: 'PROTOCOL-TEST', quantity: 1, price: 25, payment_method: 'protocol', platform: 'BENCHMARK' });
        ...
        await this.runTest(module, 'Degustare Payment (Zero Amount)', 10, async () => {
            if (this.config.orderPayloadFormat === 'hybrid') throw new Error('Degustare not supported by hybrid (POS-only feature)');
            const orderData = this.buildOrderPayload({ table: 'DEGUSTARE-TEST', quantity: 1, price: 30, payment_method: 'degustare', platform: 'BENCHMARK' });
        ...
        await this.runTest(module, 'Preț 2/3 Pricing Tiers', 10, async () => {
            if (this.config.orderPayloadFormat === 'hybrid') throw new Error('Preț 2/3 not in hybrid product schema');
            const response = await axios.get(
                `${this.config.baseURL}${this.config.endpoints.menu}`,
                { timeout: this.config.timeout }
            );
   ...
   */
    async testMobileKiosk() {
        ...
        await this.runTest(module, 'Kiosk Ordering', 10, async () => {
            const orderData = this.buildOrderPayload({ table: 'KIOSK-1', payment_method: 'card', platform: 'KIOSK' });
        ...
        await this.runTest(module, 'QR Code Table Ordering', 15, async () => {
            const orderData = this.buildOrderPayload({ table: 'QR-TABLE-5', payment_method: 'cash', platform: 'MOBILE_APP' });
    ...
    async testDeliveryDispatch() {
        ...
        await this.runTest(module, 'Order Tracking (Real-time GPS)', 10, async () => {
            const orderData = this.config.orderPayloadFormat === 'hybrid'
                ? this.buildOrderPayload()
                : this.buildOrderPayload({ type: 'delivery', payment_method: 'cash', platform: 'BENCHMARK' });
            if (this.config.orderPayloadFormat !== 'hybrid') {
                Object.assign(orderData, { customer_name: 'Test', customer_phone: '+40700000000', delivery_address: 'Test Address' });
            }
            const orderResponse = await axios.post(
                `${this.config.baseURL}${this.config.endpoints.orderCreate}`,
                orderData,
        ...
        await this.runTest(module, 'External Delivery Platforms Integration', 15, async () => {
            ...
            for (const platform of platforms) {
                try {
                    const orderData = this.config.orderPayloadFormat === 'hybrid'
                        ? this.buildOrderPayload()
                        : this.buildOrderPayload({ type: 'delivery', payment_method: 'online', platform });
                    if (this.config.orderPayloadFormat !== 'hybrid') {
                        Object.assign(orderData, { order_source: 'DELIVERY', customer_name: 'Test Customer', customer_phone: '+40700000000', delivery_address: 'Test Address', external_order_id: `${platform}-TEST-${Date.now()}` });
                    }
