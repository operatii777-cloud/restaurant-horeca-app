...
function renderTransferTemplate(doc, data) {
  const { document, fiscalHeader, lines, totals } = data;

  doc.fontSize(16).font('Helvetica-Bold').text('TRANSFER ÎNTRE GESTIUNI', { align: 'center' });
  doc.moveDown(0.5);
  ...
  if (lines && lines.length > 0) {
    doc.fontSize(11).font('Helvetica-Bold').text('Produse transferate:', { underline: true });
    doc.moveDown(0.3);
