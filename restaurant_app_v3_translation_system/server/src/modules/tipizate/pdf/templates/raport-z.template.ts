/**
 * PHASE S5.2 - Raport Z Template
 * Individual template for Raport Z (End of Day Report) documents
 */

import PDFDocument from 'pdfkit';
import { TipizatBase, TipizatLine, TipizatTotals, FiscalHeader } from '../../models/tipizate.types';

export interface RaportZTemplateData {
...
/**
 * Render Raport Z document template
 */
export function renderRaportZTemplate(doc: PDFDocument, data: RaportZTemplateData): void {
  const { document, fiscalHeader, lines, totals } = data;

  // Header
  doc.fontSize(16).font('Helvetica-Bold').text('RAPORT Z', { align: 'center' });
  doc.moveDown(0.5);
  ...
  if (lines && lines.length > 0) {
    doc.fontSize(11).font('Helvetica-Bold').text('Tranzacții finale:', { underline: true });
    doc.moveDown(0.3);
  ...
  doc.text(`TOTAL FINAL: ${totals.total?.toFixed(2) || '0.00'} RON`, { align: 'right' });
  doc.moveDown();
