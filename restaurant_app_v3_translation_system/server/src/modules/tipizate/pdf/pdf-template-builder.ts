...
class PdfTemplateBuilder {
  ...
  addWatermark(status) {
    ...
    // Fix: pages.count is the number of pages, iterate from start to start + count - 1
    for (let i = pages.start; i < pages.start + pages.count; i++) {
      try {
        this.doc.switchToPage(i);
      } catch (err) {
        // Skip if page doesn't exist
        console.warn(`[PDF] Skipping watermark on page ${i}: ${err.message}`);
        continue;
      }
      this.doc
        .fontSize(72)
  ...
  async build(data) {
    ...
    // FAZA 1.4 - Add ANAF QR and fiscal info for fiscal documents
    if (['FACTURA', 'CHITANTA', 'BON_FISCAL'].includes(document.type)) {
      await this.addAnafFiscalInfo(document);
    }
  ...
  /**
   * FAZA 1.4 - Add ANAF QR code and fiscal information
   */
  async addAnafFiscalInfo(document: any) {
    try {
      // Get ANAF submission status
      const anafStatus = await getAnafSubmissionStatus(document.type, document.id);
      const anafUniqueCode = await getAnafUniqueCode(document.type, document.id);
      const anafQRUrl = await generateAnafQRUrl(document.type, document.id);

      // Move to bottom area for QR and fiscal info
      const qrY = this.pageHeight - this.margin.bottom - 120;
      const qrSize = 80;

      // Add ANAF QR code if URL exists
      if (anafQRUrl) {
        try {
          const qrBase64 = await generateQRCodeBase64(anafQRUrl, qrSize);
          if (qrBase64) {
            // Extract base64 data
            ...
            const qrX = this.pageWidth - this.margin.right - qrSize - 10;
            this.doc.image(qrImage, qrX, qrY, { width: qrSize, height: qrSize });
            
            // Add label below QR
            this.doc.fontSize(7).font('Helvetica');
            this.doc.text('Verificare ANAF', qrX, qrY + qrSize + 5, {
              width: qrSize,
              align: 'center',
            });
          }
        } catch (qrError) {
          console.error('[PDF] Error adding QR code:', qrError);
        }
      }

      // Add fiscal information on the left
      const fiscalInfoX = this.margin.left;
      const fiscalInfoY = qrY;
      
      this.doc.fontSize(8).font('Helvetica-Bold');
      this.doc.text('Informații Fiscale ANAF:', fiscalInfoX, fiscalInfoY);
      
      this.doc.fontSize(7).font('Helvetica');
      let currentY = fiscalInfoY + 12;

      // Unique ANAF Code
      if (anafUniqueCode) {
        this.doc.text(`Cod Unic ANAF: ${anafUniqueCode}`, fiscalInfoX, currentY);
        currentY += 10;
      }
      ...
      const statusLabel = statusLabels[anafStatus.status] || anafStatus.status;
      this.doc.text(`Stare în SPV: ${statusLabel}`, fiscalInfoX, currentY);
      currentY += 10;

      // Timestamp
      if (anafStatus.timestamp) {
        const timestamp = new Date(anafStatus.timestamp).toLocaleString('ro-RO');
        this.doc.text(`Fiscalizat la: ${timestamp}`, fiscalInfoX, currentY);
      }
    } catch (error) {
      console.error('[PDF] Error adding ANAF fiscal info:', error);
      // Continue without fiscal info if there's an error
    }
  }
}
