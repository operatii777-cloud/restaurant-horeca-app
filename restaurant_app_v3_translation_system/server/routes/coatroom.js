...
// Created: 3 Dec 2025
// ENTERPRISE TODO (PHASE E5): This route will be migrated to src/modules/coatroom/routes.js
...
const runQuery = async (sql, params = []) => {
  try {
    ...
    console.error('[Coatroom] Database error:', error);
    throw error;
  }
};
...
router.post('/checkin', async (req, res) => {
  try {
    ...
    // ticket_number este obligatoriu în schema actuală
    const ticketNumber = code; // folosim același cod ca ticket_number
    
    const result = await runQuerySingle(
      `INSERT INTO coatroom_tickets (code, ticket_number, type, customer_name, notes, photo_url, created_by, status, check_in_time, items_description)
       VALUES (?, ?, ?, ?, ?, ?, ?, 'OPEN', datetime('now'), ?)`,
      [code, ticketNumber, type || 'general', customer_name || null, notes || null, photo_url || null, created_by || null, type || 'general']
    );
