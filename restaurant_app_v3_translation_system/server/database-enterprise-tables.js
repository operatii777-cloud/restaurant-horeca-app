...
function createEnterpriseTables(db) {
  ...
  // ========== FAZA 4.2 - TRAINING SYSTEM TABLES ==========
  tables.push(() => {
    db.run(`CREATE TABLE IF NOT EXISTS training_courses (
      ...
      created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
      updated_at DATETIME DEFAULT CURRENT_TIMESTAMP,
      FOREIGN KEY (requires_course_id) REFERENCES training_courses (id)
    )`, (err) => {
      if (err) console.error('Error creating training_courses:', err);
      else console.log('✅ training_courses table created');
      checkComplete();
    });
  });
  ...
  tables.push(() => {
    db.run(`CREATE TABLE IF NOT EXISTS training_modules (
      ...
      if (err) console.error('Error creating training_modules:', err);
      else console.log('✅ training_modules table created');
      checkComplete();
    });
  });

  tables.push(() => {
    db.run(`CREATE TABLE IF NOT EXISTS training_quizzes (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      ...
      if (err) console.error('Error creating training_quizzes:', err);
      else console.log('✅ training_quizzes table created');
      checkComplete();
    });
  });
  ...
  tables.push(() => {
    db.run(`CREATE TABLE IF NOT EXISTS training_progress (
      ...
      FOREIGN KEY (course_id) REFERENCES training_courses (id) ON DELETE CASCADE,
      FOREIGN KEY (module_id) REFERENCES training_modules (id) ON DELETE CASCADE,
      UNIQUE(employee_id, course_id, module_id)
    )`, (err) => {
      if (err) console.error('Error creating training_progress:', err);
      else console.log('✅ training_progress table created');
      checkComplete();
    });
  });

  tables.push(() => {
    db.run(`CREATE TABLE IF NOT EXISTS training_certifications (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      employee_id INTEGER NOT NULL,
      course_id INTEGER NOT NULL,
      course_title TEXT,
      ...
      if (err) console.error('Error creating training_certifications:', err);
      else console.log('✅ training_certifications table created');
      checkComplete();
    });
  });
  ...
  tables.push(() => {
    db.run(`CREATE TABLE IF NOT EXISTS tipizate_documents (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      type TEXT NOT NULL,
      ...
      if (err) console.error('Error creating tipizate_documents:', err);
