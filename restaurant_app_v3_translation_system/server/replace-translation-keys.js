 ...
 * Înlocuiește automat TOATE cheile de traducere ("namespace.key") cu textul în română
 *
 * INSTALARE:
...
class TranslationKeyReplacer {
  constructor(options = {}) {
    this.srcDir = options.srcDir || 'admin-vite/src';
    this.translationsFile = options.translationsFile || 'admin-vite/src/i18n/ro.json';
  ...
  async run() {
    console.log('===============================================');
    console.log('REPLACE ALL TRANSLATION STRINGS');
    console.log('===============================================\n');
  ...
  async processFile(filePath) {
    try {
      ...
      const patterns = [
        // "namespace.key" - chei de traducere cu ghilimele duble
        { regex: /"([a-zA-Z_]+\.[a-zA-Z_]+(?:\.[a-zA-Z_]+)*)"/g, capture: 1 },

        // 'namespace.key' - chei de traducere cu ghilimele simple
        { regex: /'([a-zA-Z_]+\.[a-zA-Z_]+(?:\.[a-zA-Z_]+)*)'/g, capture: 1 },
      ];
        ...
        while ((match = regex.exec(content)) !== null) {
          ...
          if (this.translations[key]) {
            ...
            if (fullMatch.startsWith('"')) {
              // String cu ghilimele duble: "namespace.key" → "translation"
              replacement = `"${this.escapeForString(translation)}"`;
            } else if (fullMatch.startsWith("'")) {
              // String cu ghilimele simple: 'namespace.key' → 'translation'
              replacement = `'${this.escapeForString(translation)}'`;
            } else {
              // Fallback
              replacement = `"${this.escapeForString(translation)}"`;
...
This script will:
  ...
  2. Find all "namespace.key" string literals
...
`);
  ...
  const replacer = new TranslationKeyReplacer({
    srcDir: 'admin-vite/src',
    translationsFile: 'admin-vite/src/i18n/ro.json',
    dryRun,
  });
