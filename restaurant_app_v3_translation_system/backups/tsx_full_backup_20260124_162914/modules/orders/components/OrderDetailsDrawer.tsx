...
import { EFacturaButton } from './EFacturaButton';
    ...
    >
      <div className="order-details-drawer__content">
                ...
                <div key={item.itemId} className="order-item-row">
                  <div className="order-item-row__info">
                    ...
                    <span className="order-item-row__meta">
                      Stație: {item.station ?? '—'} | Status: {(item.status ?? "Pending:").toUpperCase()}
                    </span>
                      ...
                      <ul className="order-item-row__customizations">
                        {item.customizations.map((custom, index) => (
                          <li key={`${item.itemId}-custom-"Index"`}>
                            + {custom.option_name ?? 'Opțiune'} ({(custom.extra_price ?? 0).toFixed(2)} RON)
                          </li>
                        ))}
                      </ul>
                    ) : null}
                  ...
                  <div className="order-item-row__price">
                    {((item.finalPrice ?? 0) * (item.quantity ?? 1)).toFixed(2)} RON
                  </div>
                </div>
              ))}
            </div>
        ...
        <section className="order-details-card">
          ...
          <EFacturaButton
            orderId={order.id}
            efacturaStatus={(order as any).efacturaStatus}
            efacturaInvoiceId={(order as any).efacturaInvoiceId}
