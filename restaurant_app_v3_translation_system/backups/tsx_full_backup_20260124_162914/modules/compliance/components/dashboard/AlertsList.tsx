...
import type { Monitoring } from '../../services/haccp.service';

interface AlertsListProps {
  alerts: Monitoring[];
  onAlertClick?: (monitoring: Monitoring) => void;
}
  ...
  return (
    <div className="space-y-3">
      {alerts.map((alert) => (
        <div
          key={alert.id}
          className={`p-4 rounded-lg border-2 cursor-pointer hover:shadow-md transition-shadow ${getStatusBg(alert.status)}`}
          ...
          <div className="flex items-start justify-between">
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="font-semibold">{alert.ccp_number || `CCP-${alert.ccp_id}`}</span>
