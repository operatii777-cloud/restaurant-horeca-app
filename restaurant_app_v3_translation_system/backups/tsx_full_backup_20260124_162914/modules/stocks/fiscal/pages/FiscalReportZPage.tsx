...
//   const { t } = useTranslation();
  ...
  const checkReportZStatus = async () => {
    ...
    try {
      ...
      const reportResponse = await httpClient.get('/api/fiscal/z-report', {
        params: {
          date: reportDate,
        },
      });

      // Check open orders count via API
      let openOrders = 0;
      let totalOrders = 0;
      try {
        const ordersResponse = await httpClient.get('/api/admin/orders/all', {
          params: {
            dateFrom: reportDate,
            dateTo: reportDate,
          },
        ...
        console.warn('Could not fetch orders for status check:', err);
      }
  ...
  const generateReportZ = async () => {
    ...
    try {
      const response = await httpClient.post('/api/fiscal/z-report', {
        date: reportDate,
      });
