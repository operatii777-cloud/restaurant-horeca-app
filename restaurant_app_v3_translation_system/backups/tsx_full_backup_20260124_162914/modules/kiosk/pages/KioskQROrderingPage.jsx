...
export const KioskQROrderingPage = () => {
  console.log('📱 KioskQROrderingPage - Component montat');
  ...
  const generateQRCode = useCallback(async () => {
    console.log('📱 KioskQROrderingPage - generateQRCode called', { restaurantUrl, location, qrSize });
    
    if (!restaurantUrl) {
      console.warn('📱 KioskQROrderingPage - restaurantUrl is empty, skipping QR generation');
      return;
    }
    ...
    console.log('📱 KioskQROrderingPage - Order URL:', orderUrl);

    try {
      // Check if QRCode is available
      if (!QRCode || typeof QRCode.toDataURL !== 'function') {
        throw new Error('QRCode library not available. Please install: npm install qrcode');
      }
      ...
      console.log('📱 KioskQROrderingPage - QR code generated successfully');
      setQrCodeDataUrl(dataUrl);
      setQrCodeUrl(orderUrl);
      setLoading(false);
    } catch (error) {
      console.error('❌ KioskQROrderingPage - Error generating QR code:', error);
      setError(error.message || 'Eroare la generarea codului QR');
      setLoading(false);
    }
  }, [restaurantUrl, location, qrSize]);

  useEffect(() => {
    console.log('📱 KioskQROrderingPage - useEffect triggered');
    // Get current domain
    const currentUrl = window.location.origin;
    console.log('📱 KioskQROrderingPage - Current URL:', currentUrl);
    if (!restaurantUrl) {
            ...
            <Card.Body className="text-center">
              {error && (
                <Alert variant="danger" className="mb-3">
                  ...
                  <br />
                  <small>Asigură-te că pachetul 'qrcode' este instalat: npm install qrcode</small>
                </Alert>
              )}
              {loading && !qrCodeDataUrl && (
                <div className="text-muted py-5">
                  <RefreshCw size={64} className="mb-3" style={{ animation: 'spin 1s linear infinite' }} />
