  ...
  if (loading) {
    return (
      <div className="mfa-page">
        ...
        <div className="mfa-page__loading">â³ Se încarcă...</div>
      </div>
    );
  }
      ...
      <div className="mfa-page__content">
        ...
        <div className="mfa-page__status-card">
          ...
          <div className="mfa-page__status">
            <span className={`mfa-page__status-badge ${status.mfaEnabled ? 'mfa-page__status-badge--enabled' : 'mfa-page__status-badge--disabled'}`}>
              {status.mfaEnabled ? 'âœ… Activ' : 'âŒ Inactiv'}
            </span>
            {status.mfaConfigured && !status.mfaEnabled && (
              <p className="mfa-page__status-note">"mfa este configurat dar nu este activat completeaz"</p>
            )}
          </div>
        </div>
        ...
        {setupStep === 'idle' && !status.mfaEnabled && (
          <div className="mfa-page__setup">
            ...
            >
              {setupMutation.loading ? '⏳ Se generează...' : '🔐 Activează MFA'}
            </button>
          </div>
        )}
        ...
        {setupStep === 'verify' && (
          <div className="mfa-page__verify">
                ...
                {secret && (
                  <div className="mfa-page__secret-backup">
                    ...
                    <p className="mfa-page__secret-note">
                      âš ï¸ Salvează acest secret într-un loc sigur pentru recovery!
                    </p>
                  </div>
                )}
              </div>
            ...
            <div className="mfa-page__actions">
              ...
              >
                {verifyMutation.loading ? 'â³ Se verifică...' : 'âœ… Verifică și Activează'}
              </button>
              <button
        ...
        {status.mfaEnabled && (
          <div className="mfa-page__disable">
            ...
            >
              {disableMutation.loading ? 'â³ Se dezactivează...' : 'âŒ Dezactivează MFA'}
            </button>
          </div>
        )}
        ...
        <div className="mfa-page__info">
          <h3>â„¹ï¸ Despre MFA</h3>
          <ul>
            <li>âœ… Protecție suplimentară pentru contul tău</li>
            <li>âœ… Cod unic la fiecare login (se schimbă la 30 secunde)</li>
            <li>âœ… Compatibil cu Google Authenticator, Microsoft Authenticator, etc.</li>
            <li>âœ… Backup secret disponibil pentru recovery</li>
          </ul>
        </div>
      </div>
    </div>
