...
//   const { t } = useTranslation();
  ...
  const loadABCAnalysis = useCallback(async () => {
    ...
    try {
      ...
      if (response.data?.success) {
        // Backend-ul returnează: { success: true, data: { summary: {...}, products: [...], ... } }
        ...
        if (Array.isArray(responseData)) {
          ...
          console.warn('❌ Format neașteptat pentru datele ABC:', responseData);
          rawProducts = [];
        }

        // Mapează proprietățile backend la interfața ABCProduct
        // Backend folosește: category (pentru ABC class), total_quantity (pentru quantity_sold)
        const data: ABCProduct[] = rawProducts.map((p: any) => ({
          ...
          abc_class: (p.abc_class || p.category || 'C') as 'A' | 'B' | 'C', // Backend folosește 'category' pentru ABC class
      ...
      console.error('❌ Eroare la încărcarea analizei ABC:', error);
  ...
  return (
    <div className="abc-analysis-page">
      <h2 className="mb-4">"analiza abc a produselor vandute"</h2>
          ...
          <Row className="mb-4">
            ...
            <Col md={4}>
              <Card className="shadow-sm">
                <Card.Header>
                  <h6>
                    <i className="fas fa-chart-pie me-1"></i>"distributie venituri"</h6>
                </Card.Header>
                <Card.Body>
                  {pieChartData ? (
                    <div style={{ height: '400px' }}>
                    ...
                    <div className="text-center text-muted">Nu există date</div>
                  )}
                </Card.Body>
              </Card>
            </Col>
