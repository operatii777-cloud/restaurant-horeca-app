...
Email: contact@trattoria.ro`);
  ...
  useEffect(() => {
    ...
    console.log(
      'âœ… ReservationsPage Filtre resetate complet - toate rezervările vor fi afișate (inclusiv cancelled și no_show)',
    );
  }, []);
    ...
    <div className="reservations-page">
      ...
      <section className="reservations-page__stats">
        <StatCard
          ...
          value={metrics ? String(metrics.today.total) : 'â€”'}
          helper="Total programate pentru azi"
          footer={
            <span>
              Confirmate: <strong>{metrics ? metrics.today.confirmed : 'â€”'}</strong> â€¢ Anulate:' '
              <strong>{metrics ? metrics.today.cancelled : 'â€”'}</strong>
            </span>
          }
        />
        <StatCard
          icon="âœ…"
          title="Confirmați"
          value={metrics ? String(metrics.stats.confirmed_reservations) : 'â€”'}
          helper="Interval selectat"
        />
        <StatCard
          icon="🚫"
          title="Anulați"
          value={metrics ? String(metrics.stats.cancelled_reservations) : 'â€”'}
          helper="Include no-show"
        />
        <StatCard
          icon="📊"
          title="Grad Ocupare"
          value={metrics ? `${metrics.occupancy.percentage}%` : 'â€”'}
          helper={
            metrics
