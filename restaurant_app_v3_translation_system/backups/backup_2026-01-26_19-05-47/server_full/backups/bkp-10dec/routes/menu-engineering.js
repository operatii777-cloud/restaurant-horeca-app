...
router.get('/analysis', async (req, res) => {
  try {
    ...
    const productMetrics = products.map(p => {
      ...
      const contributionMargin = p.revenue - foodCost;
      const cmPerUnit = p.selling_price - p.food_cost_per_unit;
      const cmPercentage = p.selling_price > 0 ? (cmPerUnit / p.selling_price) * 100 : 0;
