/**
 * Script pentru curățarea TOATE comenzilor cu status "pending" sau "preparing"
 * 
 * Acest script marchează TOATE comenzile cu status "pending" sau "preparing" ca "completed"
 * indiferent de dată, pentru a le elimina din ecranele BAR/BUCĂTĂRIE
 * 
 * Utilizare:
 * node cleanup_all_pending_orders.js
 */
...
// Funcție pentru a curăța TOATE comenzile pending/preparing
function cleanupAllPendingOrders() {
  console.log('\n🔍 Căutare TOATE comenzile cu status "pending" sau "preparing"...\n');
  ...
  db.all(query, [], (err, rows) => {
    ...
    if (rows.length === 0) {
      console.log('✅ Nu există comenzi pending/preparing!');
      db.close();
      return;
    }
    
    console.log(`📋 Găsite ${rows.length} comenzi cu status pending/preparing:\n`);
    ...
    console.log('\n⚠️  ATENȚIE: Vei marca TOATE aceste comenzi ca "completed"!');
    console.log('🔄 Marcarea comenzilor ca "completed"...\n');
