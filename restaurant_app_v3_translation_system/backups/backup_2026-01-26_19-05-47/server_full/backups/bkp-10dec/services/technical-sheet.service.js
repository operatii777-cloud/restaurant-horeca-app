...
class TechnicalSheetService {
  ...
  async create(data) {
    return new Promise((resolve, reject) => {
      ...
      db.run(`INSERT INTO technical_sheets (${fields}) VALUES (${placeholders})`, values, function(err) {
  ...
  async getById(id) {
    return new Promise((resolve, reject) => {
      db.get('SELECT * FROM technical_sheets WHERE id = ?', [id], (err, row) => {
        if (err) return reject(err);
        resolve(row);
      });
    });
  ...
  async update(id, updates) {
    return new Promise((resolve, reject) => {
      ...
      db.run(`UPDATE technical_sheets SET ${fields}, updated_at = CURRENT_TIMESTAMP WHERE id = ?`, values, (err) => {
        if (err) return reject(err);
        resolve();
      });
    });
  ...
  async logChange(techSheetId, changeType, changedBy, description) {
    ...
    return new Promise((resolve, reject) => {
      ...
      `, [techSheetId, JSON.stringify(sheet), changedBy, changeType, description], (err) => {
        if (err) return reject(err);
        resolve();
      });
    });
  ...
  async getRecipeWithIngredients(recipeId) {
    ...
    return { id: recipeId, name: 'Test Recipe', ingredients: [], portion_size: 350 };
  }
  
  async getIngredientFromCatalog(ingredientId) {
    return new Promise((resolve, reject) => {
      db.get('SELECT * FROM ingredient_catalog_global WHERE id = ?', [ingredientId], (err, row) => {
        if (err) return reject(err);
        resolve(row);
      });
    });
