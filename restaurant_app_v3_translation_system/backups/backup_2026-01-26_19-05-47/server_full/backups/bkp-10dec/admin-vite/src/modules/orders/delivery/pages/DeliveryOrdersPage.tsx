    ...
    <div className="delivery-orders-page">
      <div className="page-header">
        ...
        <div className="header-actions">
          <Button variant="success" onClick={handleCreateOrder} className="me-2">
            <i className="fas fa-plus me-1"></i>Comandă Nouă
          </Button>
      ...
      <Card className="mb-4">
        <Card.Body>
          <Row className="g-3">
            <Col md={3}>
              <Form.Label>Tip Comandă</Form.Label>
              <Tabs
                activeKey={activeTab}
                ...
                <div className="order-info mt-3">
                  ...
                  {order.car_plate && (
                    <div className="info-row">
                      ...
                      <span>Mașină: {order.car_plate} {order.lane_number && `(Lane ${order.lane_number})`}</span>
                    </div>
                  )}
                  {order.courier_name && (
                    <div className="info-row">
            ...
            <>
              ...
              <p>
                ...
                <Button variant="link" size="sm" onClick={() => callCustomer(selectedOrder.customer_phone)}>
                  <i className="fas fa-phone"></i> Sună
                </Button>
              </p>
            ...
            <>
              {!selectedOrder.is_paid && (
                <Button variant="success" onClick={() => handleMarkPaid(selectedOrder.id)}>
                  <i className="fas fa-check me-1"></i>Marchează Plătit
                </Button>
              )}
              {selectedOrder.status !== 'delivered' && selectedOrder.status !== 'completed' && (
                <Button variant="primary" onClick={() => handleMarkDelivered(selectedOrder.id)}>
                  <i className="fas fa-truck me-1"></i>Marchează Livrat
                </Button>
              )}
              {!selectedOrder.fiscal_receipt_printed && (
                <Button variant="info" onClick={() => printReceipt(selectedOrder.id)}>
                  <i className="fas fa-print me-1"></i>Printează Bon
                </Button>
              )}
      ...
      <Modal show={showCancelModal} onHide={() => setShowCancelModal(false)}>
        <Modal.Header closeButton>
          <Modal.Title>Anulează Comandă #{selectedOrder?.id}</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <Form.Group>
