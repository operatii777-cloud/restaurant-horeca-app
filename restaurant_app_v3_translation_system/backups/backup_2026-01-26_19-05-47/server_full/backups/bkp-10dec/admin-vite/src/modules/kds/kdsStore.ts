...
export const useKdsStore = create<KdsState>((get) => ({
  ...
  getPendingCount: () => {
    return get().getKitchenOrders().filter((o) => o.status === 'pending').length;
  },
  
  getPreparingCount: () => {
    return get().getKitchenOrders().filter((o) => o.status === 'preparing').length;
  },
  
  getReadyCount: () => {
    return get().getKitchenOrders().filter((o) => o.status === 'ready').length;
  },
