  ...
  return (
    <div className="daily-menu-page" data-page-ready={isPageReady}>
      <PageHeader
        ...
        description="Gestioneaza meniul zilei, programari automate ?i excep?ii pentru oferte speciale"
        actions={[
          {
            label: '? Reîmprospateaza',
            variant: 'secondary',
            onClick: refreshAll,
          },
        ]}
          ...
          <StatCard
            ...
            helper={
              currentMenu
                ...
                : 'Selecteaza ciorba ?i felul principal'
            }
          />
          <StatCard
            title="Programari active"
            value={String(schedules.length)}
            helper={`${exceptions.length} excep?ii definite`}
          />
          <StatCard
          ...
          >
            ...
            <form onSubmit={handleTodaySubmit} className="daily-menu-editor__row">
              <label>
                ...
                >
                  <option value="">-- Selecteaza ciorba --</option>
                  {soups.map((soup) => (
              ...
              <label>
                ...
                <select
                  ...
                  disabled={loading.products || saving.today}
                >
                  <option value="">-- Selecteaza felul principal --</option>
                  {mains.map((main) => (
            ...
            <div className="daily-menu-toolbar__actions" style={{ marginTop: '1rem' }}>
              <button type="button" onClick={handleTodaySubmit} disabled={saving.today || loading.products}>
                {saving.today ? 'Se salveaza...' : '?? Salveaza Meniul'}
              </button>
              {currentMenu && (
                <button type="button" onClick={handleDeactivate} disabled={saving.deactivate}>
                  {saving.deactivate ? 'Se dezactiveaza...' : '? Dezactiveaza'}
                </button>
              )}
            </div>
          </div>
            ...
            >
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                ...
                <div style={{ marginTop: '0.5rem', paddingTop: '0.5rem', borderTop: '1px solid #bbf7d0' }}>
                  <strong>Pre? total:</strong> {formatCurrency(activeMenuValue)}
                </div>
              </div>
            </div>
          )}
      ...
      {activeTab === 'calendar' && (
        <div className="daily-menu-tab" style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
          ...
          >
            ...
            <div className="daily-menu-toolbar__actions" style={{ marginTop: '1rem' }}>
              <button type="button" onClick={handleScheduleSubmit} disabled={saving.schedule || loading.products}>
                {saving.schedule ? 'Se salveaza...' : '?? Creeaza Programare'}
              </button>
            </div>
          </div>
          ...
          {schedules.length > 0 ? (
            ...
            >
              <h4 style={{ marginTop: 0, marginBottom: '1rem' }}>Programari active</h4>
              <ul className="daily-menu-placeholder">
                {schedules.map((schedule) => (
                  <li key={schedule.id}>
                    ...
                    >
                      {pendingCancelScheduleId === schedule.id ? 'Se anuleaza...' : 'Anuleaza'}
                    </button>
                  </li>
                ))}
              </ul>
            ...
            <div className="daily-menu-empty" style={{ padding: '2rem', textAlign: 'center' }}>
          ...
          {schedules.length > 0 && (
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem' }}>
              ...
              >
                <h4 style={{ marginTop: 0, marginBottom: '1rem' }}>Programari (grafic)</h4>
              ...
              >
                <h4 style={{ marginTop: 0, marginBottom: '1rem' }}>Distribu?ie</h4>
      ...
      {activeTab === 'exceptions' && (
        <div className="daily-menu-tab" style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
          ...
          >
            <h3 style={{ marginTop: 0, marginBottom: '1rem', fontSize: '1.25rem', fontWeight: 600 }}>
              Excep?ie pentru o zi specifica
            </h3>
            ...
            <div className="daily-menu-toolbar__actions" style={{ marginTop: '1rem' }}>
              <button type="button" onClick={handleExceptionSubmit} disabled={saving.exception || loading.products}>
                {saving.exception ? 'Se salveaza...' : '? Creeaza Excep?ie'}
              </button>
            </div>
          </div>
          ...
          {exceptions.length > 0 ? (
            ...
            >
              <h4 style={{ marginTop: 0, marginBottom: '1rem' }}>Excep?ii definite</h4>
              <ul className="daily-menu-placeholder">
                {exceptions.map((exception) => (
                  <li key={exception.id}>
                    ...
                    >
                      {pendingCancelExceptionId === exception.id ? 'Se anuleaza...' : 'Anuleaza'}
                    </button>
                  </li>
                ))}
              </ul>
