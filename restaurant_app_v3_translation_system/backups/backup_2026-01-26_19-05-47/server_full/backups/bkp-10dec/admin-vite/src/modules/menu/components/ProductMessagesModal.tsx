...
export function ProductMessagesModal({ open, product, onClose, onMessageSent }: ProductMessagesModalProps) {
  ...
  return (
    <Modal
      ...
      size="lg"
    >
      {loadError ? <InlineAlert variant="error" title="Nu am putut încărca istoricul" message={loadError} /> : null}
      ...
      <div className="product-messages-layout">
        <section className="product-messages-history">
          ...
          {filteredMessages.length === 0 ? (
            <div className="product-messages-empty">Nu există mesaje înregistrate pentru administrator.</div>
          ) : (
        ...
        <section className="product-messages-compose">
          <form className="product-messages-form" onSubmit={handleSubmit}>
            ...
            {sendError ? <InlineAlert variant="warning" title="Verifică mesajul" message={sendError} /> : null}

            <div className="product-messages-actions">
              <button type="button" className="menu-product-button menu-product-button--ghost" onClick={onClose} disabled={sending}>
                Închide
              </button>
