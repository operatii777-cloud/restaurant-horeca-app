  ...
  if (loading) {
    return (
      <div className="mfa-page">
        <PageHeader title="Autentificare Multi-Factor (MFA)" subtitle="Securitate suplimentară pentru contul tău" />
        <div className="mfa-page__loading">⏳ Se încarcă...</div>
      </div>
    );
  }
  ...
  return (
    <div className="mfa-page">
      <PageHeader 
        ...
        subtitle="Securitate suplimentară pentru contul tău"
      />
      ...
      <div className="mfa-page__content">
        ...
        <div className="mfa-page__status-card">
          ...
          <div className="mfa-page__status">
            <span className={`mfa-page__status-badge ${status.mfaEnabled ? 'mfa-page__status-badge--enabled' : 'mfa-page__status-badge--disabled'}`}>
              {status.mfaEnabled ? '✅ Activ' : '❌ Inactiv'}
            </span>
            {status.mfaConfigured && !status.mfaEnabled && (
              <p className="mfa-page__status-note">
                MFA este configurat dar nu este activat. Completează verificarea pentru activare.
              </p>
            )}
          </div>
        </div>
        ...
        {setupStep === 'idle' && !status.mfaEnabled && (
          <div className="mfa-page__setup">
            ...
            >
              {setupMutation.isLoading ? '⏳ Se generează...' : '🔐 Activează MFA'}
            </button>
          </div>
        )}
        ...
        {setupStep === 'verify' && (
          <div className="mfa-page__verify">
            ...
            <ol className="mfa-page__steps">
              ...
              <li>Scanează QR code-ul de mai jos</li>
                ...
                {secret && (
                  <div className="mfa-page__secret-backup">
                    ...
                    <p className="mfa-page__secret-note">
                      ⚠️ Salvează acest secret într-un loc sigur pentru recovery!
                    </p>
                  </div>
                )}
              </div>
            ...
            <div className="mfa-page__actions">
              <button
                ...
                disabled={mfaToken.length !== 6 || verifyMutation.isLoading}
              >
                {verifyMutation.isLoading ? '⏳ Se verifică...' : '✅ Verifică și Activează'}
              </button>
              <button
        ...
        {status.mfaEnabled && (
          <div className="mfa-page__disable">
            ...
            >
              {disableMutation.isLoading ? '⏳ Se dezactivează...' : '❌ Dezactivează MFA'}
            </button>
          </div>
        )}
        ...
        <div className="mfa-page__info">
          <h3>ℹ️ Despre MFA</h3>
          <ul>
            <li>✅ Protecție suplimentară pentru contul tău</li>
            <li>✅ Cod unic la fiecare login (se schimbă la 30 secunde)</li>
            <li>✅ Compatibil cu Google Authenticator, Microsoft Authenticator, etc.</li>
            <li>✅ Backup secret disponibil pentru recovery</li>
          </ul>
        </div>
      </div>
    </div>
