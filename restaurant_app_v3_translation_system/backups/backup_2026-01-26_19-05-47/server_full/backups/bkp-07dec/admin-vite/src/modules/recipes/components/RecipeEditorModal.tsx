          ...
          <div className="recipe-yield-config" style={{ marginTop: '0.5rem', padding: '0.5rem', background: '#f5f5f5', borderRadius: '4px' }}>
            <label style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '0.875rem' }}>
              ...
              <input 
                ...
                onChange={(e) => {
                  // TODO: Save servings to product
                  console.log('Servings changed:', e.target.value);
                }}
              />
            </label>
            <div style={{ fontSize: '0.75rem', color: '#666', marginTop: '0.25rem' }}>
              Cost per serving: {suggestedPrice ? (suggestedPrice.totalCost / (parseInt((document.getElementById('recipe-servings') as HTMLInputElement)?.value || '1'))).toFixed(2) : 'N/A'} RON
            </div>
          </div>
        </div>
