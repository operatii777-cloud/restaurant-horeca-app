  ...
  return (
    <div className="couriers-page" data-page-ready="true">
      ...
      <div className="couriers-summary">
        ...
        <div className="summary-item busy"><span className="summary-value">{couriers.filter(c => c.status === 'busy').length}</span><span className="summary-label">În Livrare</span></div>
        <div className="summary-item"><span className="summary-value">{couriers.reduce((sum, c) => sum + (c.today_deliveries || 0), 0)}</span><span className="summary-label">Livrări Azi</span></div>
      </div>
        ...
        <div className="modal-overlay" onClick={() => setShowAddModal(false)}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={(e) => { e.preventDefault(); handleAddCourier(); }}>
              ...
              <div className="form-group">
                <label>Email</label>
                <input type="email" value={newCourier.email} onChange={(e) => setNewCourier({ ...newCourier, email: e.target.value })} placeholder="ion@email.com" />
              </div>

              <div className="form-group">
                <label>Vehicul</label>
                <select value={newCourier.vehicle_type} onChange={(e) => setNewCourier({ ...newCourier, vehicle_type: e.target.value })}>
