...
export function BulkPriceModal({ open, productCount, productIds, onClose, onApplied }: BulkPriceModalProps) {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (response !== null) {
      onApplied(response.updated_count ?? productIds.length, payload.new_price as number | undefined, payload.new_vat_rate as number | undefined);
    }
  };
  ...
  return (
    <Modal
      isOpen={open}
      title="Schimbare preț în masă"
      description={`Vor fi actualizate ${productCount} produse selectate.`}
      size="md"
      onClose={onClose}
    >
      {localError ? <InlineAlert variant="warning" title="Verifică datele" message={localError} /> : null}
      ...
      <form className="bulk-price-form" onSubmit={handleSubmit}>
        <label className="bulk-price-field">
          ...
          <input
            ...
            placeholder="Lasă gol pentru a păstra prețul"
          />
        </label>
        ...
        <label className="bulk-price-field">
          ...
          <input
            ...
            placeholder="Lasă gol pentru a păstra cota existentă"
          />
        </label>
