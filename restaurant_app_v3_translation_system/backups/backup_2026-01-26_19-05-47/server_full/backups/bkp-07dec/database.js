...
const dbPromise = new Promise((resolve, reject) => {
  const db = new sqlite3.Database(DB_PATH, (err) => {
    if (err) {
      ...
      // Configurare timeout pentru a preveni blocarea
      db.configure('busyTimeout', 5000); // 5 secunde
      
      // Activează foreign keys permanent (trebuie să aștepte finalizarea)
      db.run("PRAGMA foreign_keys = ON", (err) => {
        if (err) {
          console.error('⚠️ Eroare la activarea foreign_keys:', err.message);
          // Continuă chiar dacă eșuează
        } else {
          console.log('✅ Foreign keys activate permanent');
        }
...
      db.run(`CREATE TABLE IF NOT EXISTS app_settings (key TEXT PRIMARY KEY, value TEXT)`);
      ...
      const waiterColumnsMigration = [
        ...
        `ALTER TABLE waiters ADD COLUMN updated_at DATETIME DEFAULT CURRENT_TIMESTAMP`,
      ...
      db.run(`CREATE TABLE IF NOT EXISTS stock_moves (
        ...
        type TEXT NOT NULL CHECK (type IN ('NIR', 'CONSUME', 'ADJUST')),
        reference_type TEXT,
      ...
      db.run(`
        ...
        BEGIN
          ...
          INSERT INTO recipe_recalculation_queue (product_id, reason, created_at)
          SELECT DISTINCT r.product_id, 'ingredient_cost_changed', datetime('now')
          FROM recipes r
      ...
      db.run(`
        ...
        BEGIN
          ...
          INSERT INTO recipe_recalculation_queue (product_id, reason, created_at)
          SELECT DISTINCT r.product_id, 'sub_recipe_cost_changed', datetime('now')
          FROM recipes r
