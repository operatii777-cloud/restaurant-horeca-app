...
function validateInvoiceData(invoiceData) {
  ...
  // Validate supplier
  if (!invoiceData.supplier.name || !invoiceData.supplier.cui) {
  ...
  invoiceData.invoiceLines.forEach((line, idx) => {
    if (!line.name || !line.quantity || !line.unitPrice) {
