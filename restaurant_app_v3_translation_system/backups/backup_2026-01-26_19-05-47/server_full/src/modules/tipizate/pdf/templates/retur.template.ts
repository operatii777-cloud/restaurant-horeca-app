/**
 * PHASE S5.2 - Retur Template
 * Individual template for Retur (Return) documents
 */

import PDFDocument from 'pdfkit';
import { TipizatBase, TipizatLine, TipizatTotals, FiscalHeader } from '../../models/tipizate.types';

export interface ReturTemplateData {
...
/**
 * Render Retur document template
 */
export function renderReturTemplate(doc: PDFDocument, data: ReturTemplateData): void {
  const { document, fiscalHeader, lines, totals } = data;

  // Header
  doc.fontSize(16).font('Helvetica-Bold').text('DOCUMENT DE RETUR', { align: 'center' });
  doc.moveDown(0.5);
  ...
  if (lines && lines.length > 0) {
    doc.fontSize(11).font('Helvetica-Bold').text('Produse returnate:', { underline: true });
    doc.moveDown(0.3);
    ...
    lines.forEach((line) => {
      ...
      doc.text((line as any).reason || 'N/A', reasonX, y);
  ...
  doc.text(`TOTAL RETUR: ${totals.total?.toFixed(2) || '0.00'} RON`, { align: 'right' });
  doc.moveDown();

  // Semnături
  doc.moveDown(2);
  doc.fontSize(9).font('Helvetica');
  doc.text('Returnat de:', 50, doc.y);
