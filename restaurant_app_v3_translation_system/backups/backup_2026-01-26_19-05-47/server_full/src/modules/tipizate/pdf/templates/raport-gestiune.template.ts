/**
 * PHASE S5.2 - Raport de Gestiune Template
 * Individual template for Raport de Gestiune documents
 */

import PDFDocument from 'pdfkit';
import { TipizatBase, TipizatLine, TipizatTotals, FiscalHeader } from '../../models/tipizate.types';

export interface RaportGestiuneTemplateData {
...
/**
 * Render Raport de Gestiune document template
 */
export function renderRaportGestiuneTemplate(doc: PDFDocument, data: RaportGestiuneTemplateData): void {
  const { document, fiscalHeader, lines, totals } = data;

  // Header
  doc.fontSize(16).font('Helvetica-Bold').text('RAPORT DE GESTIUNE', { align: 'center' });
  doc.moveDown(0.5);
  ...
  if (lines && lines.length > 0) {
    doc.fontSize(11).font('Helvetica-Bold').text('Mișcări stoc:', { underline: true });
    doc.moveDown(0.3);
