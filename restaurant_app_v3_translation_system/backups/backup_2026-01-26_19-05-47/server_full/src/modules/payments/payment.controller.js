...
class PaymentController {
  ...
  async createPayment(req, res) {
    try {
      const { id } = req.params;
      const { amount, currency, method, provider, reference, meta, idempotency_key } = req.body;
      ...
      const payment = await paymentService.createPayment(
        ...
        idempotency_key || req.headers['idempotency-key'] // Suport pentru header sau body
      );

      // Adaugă idempotency key în response dacă e duplicate
