...
class CompleteTextScanner {
  constructor(options = {}) {
    ...
    this.translationsFile = options.translationsFile || './src/i18n/locales/ro.json';
  ...
  extractIdentifiers(content, filePath) {
    ...
    const patterns = [
      ...
      /function\s+([a-z][a-zA-Z0-9]*(?:Label|Title|Text|Message|Name))/gi,
    ];
...
async function main() {
  const scanner = new CompleteTextScanner({
    srcDir: './src',
    translationsFile: './src/i18n/locales/ro.json',
  });
