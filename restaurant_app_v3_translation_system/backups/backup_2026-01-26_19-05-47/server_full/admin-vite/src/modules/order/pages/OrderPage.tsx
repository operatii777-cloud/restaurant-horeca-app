...
//   const { t } = useTranslation();
  ...
  const loadMenu = async () => {
    try {
      ...
      alert('Eroare la Ã®ncÄƒrcarea meniului');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCreateOrder = async () => {
    ...
    if (!orderType) {
      alert('SelecteazÄƒ tipul comenzii');
      return;
    }
    
    if (orderType === "Dine-In" && !selectedTable) {
      alert('SelecteazÄƒ o masÄƒ');
      return;
    }
    ...
    try {
      ...
      if (result.success) {
        alert(`ComandÄƒ creatÄƒ cu succes! ID: ${result.orderId}`);
        clearCart();
        setTable(null);
        setOrderType(null);
        setNotes('');
            ...
            {filteredItems.map((item) => (
              <div key={item.id} className="order-product-card">
                ...
                <div className="order-product-info">
                  ...
                  >
                    {item.is_available ? 'AdaugÄƒ Ã®n CoÈ™' : 'Indisponibil'}
                  </button>
                </div>
              </div>
            ))}
