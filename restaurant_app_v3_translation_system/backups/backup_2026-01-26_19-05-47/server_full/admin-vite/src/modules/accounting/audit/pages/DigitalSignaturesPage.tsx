 ...
 * 
 * Gestionare SemnÄƒturi Digitale pentru Documente Contabile:
 * - Lista semnÄƒturi
...
//   const { t } = useTranslation();
  ...
  const loadSignatures = async () => {
    ...
    try {
      ...
      setError(err.response?.data?.error || err.message || 'Eroare la Ã®ncÄƒrcarea semnÄƒturilor');
      setSignatures([]);
    } finally {
      setLoading(false);
    }
  ...
  const handleVerifySignature = async (id: number) => {
    try {
      ...
      if (response.data.success) {
        ...
        alert('Eroare la verificare: ' + (response.data.error || 'Eroare necunoscutÄƒ'));
      }
    } catch (err: any) {
  ...
  const getDocumentTypeBadge = (type: string) => {
    const badges: Record<string, { bg: string; label: string }> = {
      'invoice': { bg: 'primary', label: 'FacturÄƒ' },
  ...
  return (
    <div className="digital-signatures-page">
      <div className="page-header d-flex justify-content-between align-items-center">
        <div>
          <h1>âœï¸ SemnÄƒturi Digitale</h1>
        ...
        <HelpButton
          ...
          content={
            <div>
              <h5>âœï¸ Ce sunt semnÄƒturile digitale?</h5>
              <p>
                SemnÄƒturile digitale asigurÄƒ integritatea È™i autenticitatea documentelor contabile. 
                Fiecare document important (facturi, rapoarte Z, etc.) poate fi semnat digital.
              </p>
              <h5 className="mt-4">ðŸ” FuncÈ›ionalitÄƒÈ›i</h5>
              <ul>
                <li><strong>"generare semnatura"</strong> - CreeazÄƒ o semnÄƒturÄƒ digitalÄƒ pentru un document</li>
                <li><strong>"verificare semnatura"</strong> - VerificÄƒ dacÄƒ un document a fost modificat</li>
                <li><strong>Istoric</strong> - Vezi toate semnÄƒturile generate</li>
                <li><strong>"Detalii"</strong> - Vezi informaÈ›ii despre semnÄƒturÄƒ (hash, timestamp, etc.)</li>
              </ul>
              <h5 className="mt-4">ðŸ”’ Securitate</h5>
              <p>
                SemnÄƒturile digitale folosesc algoritmi criptografici pentru a asigura cÄƒ documentul 
                nu a fost modificat dupÄƒ semnare. Orice modificare va invalida semnÄƒtura.
              </p>
              <div className="alert alert-warning mt-4">
                <strong>âš ï¸ Important:</strong> SemnÄƒturile digitale nu Ã®nlocuiesc semnÄƒturile legale. 
                ConsultÄƒ Ã®ntotdeauna un avocat pentru aspecte legale.
              </div>
            </div>
          }
        />
                      ...
                      <tr key={signature.id}>
                        ...
                        <td>
                          <Badge bg={signature.is_valid ? 'success' : 'danger'}>
                            {signature.is_valid ? 'ValidÄƒ' : 'InvalidÄƒ'}
                          </Badge>
                        </td>
                        <td>
                          <Button
              ...
              <Form.Group className="mb-3">
                ...
                <div>
                  <Badge bg={selectedSignature.is_valid ? 'success' : 'danger'}>
                    {selectedSignature.is_valid ? 'ValidÄƒ' : 'InvalidÄƒ'}
                  </Badge>
                </div>
              </Form.Group>
