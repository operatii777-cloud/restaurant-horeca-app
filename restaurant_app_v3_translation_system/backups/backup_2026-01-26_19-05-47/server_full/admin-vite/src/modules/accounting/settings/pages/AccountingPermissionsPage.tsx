...
//   const { t } = useTranslation();
  ...
  const loadData = async () => {
    ...
    try {
      ...
      setError(err.response?.data?.error || err.message || 'Eroare la Ã®ncÄƒrcarea datelor');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleRemovePermission = async (userId: number, permissionId: number) => {
    if (!confirm('Sigur doriÈ›i sÄƒ eliminaÈ›i aceastÄƒ permisiune?')) {
      return;
    }
    ...
    try {
      ...
      if (response.data.success) {
        ...
        alert('Eroare la eliminare: ' + (response.data.error || 'Eroare necunoscutÄƒ'));
      }
    } catch (err: any) {
  ...
  const handleSubmitAssign = async () => {
    if (!selectedUser || !selectedPermission) {
      alert('SelectaÈ›i utilizatorul È™i permisiunea');
      return;
    }
    ...
    try {
      ...
      if (response.data.success) {
        ...
        alert('Eroare la asignare: ' + (response.data.error || 'Eroare necunoscutÄƒ'));
      }
    } catch (err: any) {
  ...
  const getPermissionCategoryBadge = (category: string) => {
    const badges: Record<string, { bg: string; label: string }> = {
      ...
      'settings': { bg: 'warning', label: 'SetÄƒri' },
          ...
          <Card className="mb-4">
            <Card.Header className="d-flex justify-content-between align-items-center">
              ...
              <Button variant="outline-primary" size="sm" onClick={loadPermissions}>
                <i className="fas fa-sync me-2"></i>"ReÃ®ncarcÄƒ"</Button>
            </Card.Header>
            <Card.Body>
              {loading ? (
                          ...
                          <tr key={permission.id}>
                            ...
                            <td>
                              <Badge bg={permission.is_active ? 'success' : 'secondary'}>
                                {permission.is_active ? 'ActivÄƒ' : 'InactivÄƒ'}
                              </Badge>
                            </td>
                          </tr>
                        ));
                      ...
                      return (
                        <tr key={user.id}>
                          ...
                          <td>
                            ...
                            >
                              <i className="fas fa-plus me-1"></i>"AsigneazÄƒ"</Button>
                          </td>
                        </tr>
                      );
                    });
      ...
      <Modal show={showAssignModal} onHide={() => {
        ...
        <Modal.Body>
          {selectedUser && (
            <div>
              ...
              <Form.Group className="mb-3">
                <Form.Label><strong>SelecteazÄƒ Permisiune</strong></Form.Label>
                <Form.Select
        ...
        <Modal.Footer>
          ...
          <Button variant="primary" onClick={handleSubmitAssign} disabled={!selectedPermission}>
            <i className="fas fa-check me-2"></i>"AsigneazÄƒ"</Button>
        </Modal.Footer>
      </Modal>
      ...
      <Modal show={showHelpModal} onHide={() => setShowHelpModal(false)} size="lg" className="help-modal">
        ...
        <Modal.Body>
          <div className="help-content">
            <h5>ðŸ“‹ Cum funcÈ›ioneazÄƒ permisiunile?</h5>
            <p>
              Permisiunile contabilitate controleazÄƒ accesul utilizatorilor la funcÈ›iile modulului contabilitate.
            ...
            <ol>
              <li>
                ...
                <ul>
                  <li>"mergi la"<strong>SetÄƒri â†’ Utilizatori & Permisiuni</strong></li>
                  <li>"sau acceseaza direct"<code>/admin-vite/settings/users</code></li>
                </ul>
              </li>
              <li>
                <strong>Click pe butonul "âž• AdaugÄƒ Utilizator"</strong>
              </li>
              <li>
                <strong>"completeaza formularul"</strong>
                <ul>
                  <li><strong>Username</strong> (obligatoriu) - numele de utilizator</li>
                  <li><strong>Email</strong> (opÈ›ional) - adresa de email</li>
                  <li><strong>ParolÄƒ</strong> (obligatoriu) - parola pentru autentificare</li>
                  <li><strong>Rol</strong> (obligatoriu) - selecteazÄƒ rolul din dropdown</li>
                </ul>
              </li>
              <li>
                <strong>Click pe "SalveazÄƒ"</strong>
              </li>
            </ol>
            ...
            <div className="alert alert-info">
              <ol className="mb-0">
                <li><strong>Creezi utilizatorul</strong> â†’ SetÄƒri â†’ Utilizatori & Permisiuni</li>
                <li><strong>Utilizatorul apare</strong> Ã®n lista de utilizatori</li>
                <li><strong>"mergi la"</strong> Contabilitate â†’ SetÄƒri â†’ Permisiuni Contabilitate</li>
                <li><strong>Click pe "AsigneazÄƒ"</strong> lÃ¢ngÄƒ utilizatorul dorit</li>
                <li><strong>"selectezi permisiunea"</strong> din dropdown</li>
                <li><strong>Click "AsigneazÄƒ"</strong> pentru a finaliza</li>
              </ol>
            </div>
            ...
            <ul>
              ...
              <li><strong>"gestionare setari"</strong> - Poate modifica setÄƒrile contabilitate</li>
            ...
            <div className="alert alert-warning mt-4">
              <strong>ðŸ’¡ NotÄƒ:</strong> Utilizatorii creaÈ›i Ã®n pagina "Utilizatori & Permisiuni" vor apÄƒrea automat 
              Ã®n aceastÄƒ paginÄƒ pentru asignarea permisiunilor specifice contabilitÄƒÈ›ii.
            </div>
          </div>
        </Modal.Body>
        <Modal.Footer>
