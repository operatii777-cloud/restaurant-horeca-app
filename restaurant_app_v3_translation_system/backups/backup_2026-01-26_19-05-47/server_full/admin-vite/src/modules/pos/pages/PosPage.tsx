...
//   const { t } = useTranslation();
  ...
  const handleFiscalize = async () => {
    if (!currentOrderId) {
      setFiscalError('Nu existÄƒ o comandÄƒ activÄƒ');
    ...
    if (!isReadyForFiscalization) {
      setFiscalError('Comanda nu este plÄƒtitÄƒ complet');
      setFiscalErrorType('generic');
      setShowFiscalError(true);
      return;
    }
    ...
    if (isFiscalized) {
      setFiscalError('Comanda este deja fiscalizatÄƒ');
      setFiscalErrorType('generic');
      setShowFiscalError(true);
      return;
    }
    ...
    try {
      ...
      if (errorCode.includes('PRINTER') || errorMessage.toLowerCase().includes('printer')) {
        ...
        setFiscalError('Imprimanta fiscalÄƒ este offline sau nu rÄƒspunde. VerificÄƒ conexiunea È™i Ã®ncearcÄƒ din nou.');
      } else if (errorCode.includes('ANAF') || errorMessage.toLowerCase().includes('anaf')) {
        setFiscalErrorType('anaf');
        setFiscalError('Serviciul ANAF este indisponibil momentan. ÃŽncearcÄƒ din nou Ã®n cÃ¢teva momente.');
        ...
        setFiscalError('Lipsesc coduri fiscale pentru unele produse. VerificÄƒ nomenclatorul produselor.');
      } else {
        setFiscalErrorType('generic');
  ...
  const handleCloseOrder = async () => {
    ...
    if (!isFiscalized) {
      if (!confirm('Comanda nu este fiscalizatÄƒ. EÈ™ti sigur cÄƒ vrei sÄƒ Ã®nchizi comanda?')) {
        return;
      }
    }
    ...
    <div className="pos-page">
      ...
      <Modal show={showFiscalError} onHide={() => setShowFiscalError(false)} centered>
        <Modal.Header closeButton>
          <Modal.Title>
            {fiscalErrorType === 'printer' && 'âš ï¸ ImprimantÄƒ Offline'}
            {fiscalErrorType === 'anaf' && 'âš ï¸ ANAF Indisponibil'}
            {fiscalErrorType === "Nomenclator" && 'âš ï¸ Coduri Fiscale LipsÄƒ'}
        ...
        <Modal.Body>
          ...
          {fiscalErrorType === 'printer' && (
            <div className="mt-3">
              <p className="text-muted small">"VerificÄƒ:"</p>
              <ul className="text-muted small">
                <li>"conexiunea la imprimanta"</li>
                <li>Statusul imprimantei (hÃ¢rtie, erori)</li>
          ...
          {fiscalErrorType === "Nomenclator" && (
            <div className="mt-3">
              <p className="text-muted small">
                Unele produse nu au coduri fiscale (PLU) configurate. AdaugÄƒ codurile Ã®n catalogul de produse.
              </p>
            </div>
          )}
        </Modal.Body>
