...
//   const { t } = useTranslation();
  ...
  const loadTables = async () => {
    try {
      ...
      setError(err.response?.data?.error || 'Eroare la Ã®ncÄƒrcarea meselor');
    } finally {
      setLoading(false);
    }
  };
    ...
    return (
      <div className="pos-table-selector-error">
        ...
        <button className="btn btn-outline-primary btn-sm" onClick={loadTables}>
          <i className="fas fa-redo me-1"></i>"ReÃ®ncearcÄƒ"</button>
      </div>
    );
  }
  ...
  return (
    <div className="pos-table-selector">
      <div className="pos-table-selector-header">
        <h4 className="pos-table-selector-title">SelecteazÄƒ Masa</h4>
        <button
