...
//   const { t } = useTranslation();
  ...
  const loadStats = async () => {
    ...
    try {
      ...
      setError(err.response?.data?.error || 'Eroare la Ã®ncÄƒrcarea statisticilor.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const revenueChartOptions = {
    ...
    plugins: {
      ...
      title: {
        display: true,
        text: 'EvoluÈ›ie Venituri & Discount Happy Hour',
      },
    },
    scales: {
      y: {
  ...
  const topProductsChartOptions = {
    ...
    plugins: {
      ...
      title: {
        display: true,
        text: 'Top 10 Produse VÃ¢ndute Ã®n Happy Hour',
      },
    },
    scales: {
      y: {
          ...
          <Row className="mb-3">
            ...
            <Col md={4} className="d-flex align-items-end">
              <Button variant="primary" onClick={loadStats} disabled={loading}>
                {loading ? (
                  <>
                    <Spinner animation="border" size="sm" className="me-2" />"se incarca"</>
                ) : (
                  <>
                    <i className="fas fa-sync me-2"></i>"ReÃ®ncarcÄƒ"</>
                )}
              </Button>
            </Col>
          </Row>
