...
// InterfaÈ›Äƒ completÄƒ pentru gestionarea comenzilor delivery
// =====================================================================
...
//   const { t } = useTranslation();
  ...
  const cancelOrder = async () => {
    if (!selectedOrder || !cancelReason) {
      alert('SelecteazÄƒ un motiv de anulare');
      return;
    }
    ...
    try {
      ...
      if (data.success) {
        ...
        setCancelReason('');
        fetchOrders();
        alert('ComandÄƒ anulatÄƒ cu succes');
      } else {
        alert('Eroare: ' + data.error);
      }
    } catch (err) {
      alert('Eroare la anulare comandÄƒ');
    }
  };
    ...
    <div className="delivery-dashboard-page">
          ...
          <div className="filters-row">
            <Form.Group>
              ...
              <Form.Select value={filterStatus} onChange={(e) => setFilterStatus(e.target.value)}>
                ...
                <option value="assigned">"AlocatÄƒ"</option>
                <option value="picked_up">"PreluatÄƒ"</option>
        ...
        <Card.Body>
                ...
                <tr key={order.id} className={getWaitTime(order.timestamp) > 30 ? 'table-warning' : ''}>
                  ...
                  <td>
                    <div className="action-buttons">
                      ...
                      <Button size="sm" variant="primary" onClick={() => callCustomer(order.customer_phone)} title="SunÄƒ">
                        ðŸ“ž
                      </Button>
                      ...
                      <Button size="sm" variant="danger" onClick={() => openCancelModal(order)} title="AnuleazÄƒ">
                        Ã¢ÂÅ’
                      </Button>
                    </div>
                  </td>
          ...
          {filteredOrders.length === 0 && (
            <div className="text-center text-muted py-5">
              <h5>"Nicio comandÄƒ delivery activÄƒ"</h5>
            </div>
          )}
        </Card.Body>
      </Card>
      ...
      <Modal show={showDetailsModal} onHide={() => setShowDetailsModal(false)} size="lg">
        <Modal.Header closeButton>
          <Modal.Title>Detalii ComandÄƒ #{selectedOrder?.id}</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          {selectedOrder && (
            <div>
              ...
              <h5>"InformaÈ›ii Livrare"</h5>
              <p>
                <strong>"PlatformÄƒ:"</strong> {getPlatformIcon(selectedOrder.platform)} {selectedOrder.platform}<br />
                <strong>"PlatÄƒ:"</strong> {selectedOrder.payment_method}<br />
      ...
      <Modal show={showCancelModal} onHide={() => setShowCancelModal(false)}>
        <Modal.Header closeButton>
          <Modal.Title>Anulare ComandÄƒ #{selectedOrder?.id}</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <Form.Group>
