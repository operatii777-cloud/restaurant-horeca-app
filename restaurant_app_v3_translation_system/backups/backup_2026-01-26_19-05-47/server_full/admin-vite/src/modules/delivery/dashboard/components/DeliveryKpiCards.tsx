...
/**
 * FAZA 4.4 - Delivery KPI Cards Component
 * 
 * Connected to real API data
 * Displays: avg delivery time, avg pickup time, SLA % on-time, top courier, platform mix
 */
...
export function DeliveryKpiCards({ overview, loading }: DeliveryKpiCardsProps) {
//   const { t } = useTranslation();
  ...
  const avgPickupTime = overview.avgAssignMinutes || 0;
  const avgTransitTime = overview.avgTransitMinutes || 0;
  const topPlatform = overview.platformMix && overview.platformMix.length > 0
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
      ...
      <div className="bg-white rounded-lg p-6 shadow hover:shadow-lg transition-shadow">
        ...
        <div className={`text-3xl font-bold ${
          overview.onTimeRate >= 0.9 ? 'text-green-600' :
      ...
      <div className="bg-white rounded-lg p-6 shadow hover:shadow-lg transition-shadow">
        ...
        {overview.platformMix && overview.platformMix.length > 0 && (
