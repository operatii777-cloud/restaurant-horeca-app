...
//   const { t } = useTranslation();
  ...
  const handleOpenDetails = useCallback(
    (tab: IngredientDetailsTab = 'overview') => {
      if (!primaryIngredient) {
        setFeedback({ type: 'warning', message: 'SelecteazÄƒ un ingredient pentru a vizualiza detaliile.' });
        return;
      }
      setDetailsTab(tab);
      setDetailsOpen(true);
  ...
  const handleOpenBulkModal = useCallback(() => {
    if (!selectionCount) {
      setFeedback({ type: 'warning', message: 'SelecteazÄƒ cel puÈ›in un ingredient pentru actualizare Ã®n masÄƒ.' });
      return;
    }
  ...
  const handleBulkApplied = useCallback(
    async ({ updatedCount, visibilityAction, minStock, costPerUnit }: { updatedCount: number; visibilityAction?: 'hide' | 'restore' | null; minStock?: number | null; costPerUnit?: number | null }) => {
      const parts: string[] = [`Actualizare aplicatÄƒ pentru ${updatedCount} ingrediente.`];
      ...
      if (visibilityAction === 'hide') {
        ...
        parts.push('Ingredientele au fost restaurate Ã®n gestiune.');
      }
  ...
  const handleHideSelected = useCallback(async () => {
    if (!selectionCount) {
      setFeedback({ type: 'warning', message: 'SelecteazÄƒ ingredientele pe care doreÈ™ti sÄƒ le ascunzi.' });
      return;
    }
    try {
      ...
      setFeedback({ type: 'error', message: apiError?.response?.data?.error ?? 'Nu s-a putut finaliza acÈ›iunea.' });
    }
  }, [selectedIngredients, selectionCount, refetch]);

  const handleRestoreSelected = useCallback(async () => {
    if (!selectionCount) {
      setFeedback({ type: 'warning', message: 'SelecteazÄƒ ingredientele pe care doreÈ™ti sÄƒ le restaurezi.' });
      return;
    }
    try {
      ...
      setFeedback({ type: 'error', message: apiError?.response?.data?.error ?? 'Nu s-a putut finaliza acÈ›iunea de restaurare.' });
    }
  }, [selectedIngredients, selectionCount, refetch]);

  const handleExportCsv = useCallback(() => {
    if (!filteredData.length) {
      setFeedback({ type: 'warning', message: 'Nu existÄƒ date de exportat.' });
      return;
    }
    ...
    <div className="ingredients-page" data-page-ready={isPageReady ? 'true' : 'false'}>
      <section className="ingredients-hero">
        <div className="ingredients-hero__info">
          ...
          <h2>Inventar operaÈ›ional â€“ Ingrediente</h2>
          <p>
            Vizualizezi Ã®n timp real stocurile, temperaturile controlate È™i documentele asociate fiecÄƒrui ingredient.
            AG Grid oferÄƒ filtrare avansatÄƒ, sortare È™i export Excel.
          </p>
        </div>
        ...
        <div className="ingredients-hero__stats">
          <StatCard
            ...
            helper="Disponibile Ã®n meniuri È™i reÈ›ete"
            value={`${activeIngredients}`}
            trendLabel="vs. sÄƒptÄƒmÃ¢na trecutÄƒ"
            trendValue="â–² 12%"
            trendDirection="up"
            icon={<span>âœ…</span>}
          />
          ...
          <StatCard
            ...
            helper="FiÈ™e actualizate Ã®n ultimele 30 zile"
            value="96% conform"
        ...
        <div className="ingredients-hero__analytics">
          <div className="ingredients-analytics-card">
            <header>
              ...
              <span className="ingredients-analytics-helper">kg utilizaÈ›i / zi</span>
            </header>
      ...
      <section className="ingredients-toolbar" aria-label="filtre si actiuni">
        <div className="ingredients-toolbar__left">
          ...
          <label className="ingredients-toggle">
            ...
            />
            AfiÈ™eazÄƒ ingredientele ascunse ({hiddenIngredients})
          </label>
        </div>
      ...
      <div className="ingredients-feedback">
        {feedback ? (
          <InlineAlert
            ...
            title={feedback.type === 'success' ? 'Succes' : feedback.type === 'error' ? 'Eroare' : 'InformaÈ›ie'}
      ...
      <section className="ingredients-grid-panel">
        <header>
          <div>
            ...
            <p>{`${totalIngredients} ingrediente gestionate Â· Mod filtrare rapidÄƒ activ`}</p>
          </div>
      ...
      <section className="ingredients-secondary">
        ...
        <article className="ingredients-secondary__card">
          <header>
            <span>AutomatizÄƒri & alerte</span>
            <button type="button" className="ingredients-link-button">
              ConfigureazÄƒ notificÄƒri â†’
            </button>
          </header>
          <ul>
            <li>
              <strong>Alerte stoc critic</strong>
              <span>Livrare Ã®n 6h</span>
            </li>
            <li>
