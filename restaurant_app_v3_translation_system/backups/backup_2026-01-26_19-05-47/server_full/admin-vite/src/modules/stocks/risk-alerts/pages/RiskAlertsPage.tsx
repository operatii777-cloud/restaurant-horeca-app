...
//   const { t } = useTranslation();
  ...
  const loadRiskData = async () => {
    try {
      ...
      if (data.summary?.high_risk_products > 0) {
        recs.push(`Urgent: ${data.summary.high_risk_products} produse cu risc ridicat necesitÄƒ reabastecere imediatÄƒ`);
      }
      if (data.summary?.total_cancelled_value > 1000) {
        recs.push(`Valoare pierdutÄƒ semnificativÄƒ: ${data.summary.total_cancelled_value.toFixed(2)} RON din anulÄƒri`);
      }
  ...
  const columnDefs = [
    ...
    { field: 'total_cancellations', headerName: 'AnulÄƒri Totale', width: 150 },
    { field: 'stock_related_cancellations', headerName: 'AnulÄƒri Stoc', width: 150 },
    { field: 'cancelled_value', headerName: 'Valoare PierdutÄƒ', width: 150, valueFormatter: (params: any) => `${params.value.toFixed(2)} RON` },
    {
      field: 'risk_level',
      headerName: 'Risc',
      width: 120,
        ...
        <div className="card mb-4">
          ...
          <div className="card-body">
            <div className="row">
              ...
              <div className="col-md-3">
                <p><strong>Valoare TotalÄƒ PierdutÄƒ:</strong> <span className="text-danger">{summary.total_cancelled_value.toFixed(2)} RON</span></p>
              </div>
              <div className="col-md-3">
