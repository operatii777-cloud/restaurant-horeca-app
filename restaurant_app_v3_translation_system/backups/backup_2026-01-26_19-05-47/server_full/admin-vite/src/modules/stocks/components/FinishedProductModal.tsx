  ...
  useEffect(() => {
    ...
    const fetchCatalogProducts = async () => {
      try {
        ...
        console.error('âŒ Eroare la Ã®ncÄƒrcarea catalogului de produse:', error);
        if (isMounted) {
          setFeedback({ type: 'error', message: 'Nu s-a putut Ã®ncÄƒrca lista produselor din meniu.' });
        }
      }
    };
    ...
    const fetchExistingStock = async () => {
      ...
      try {
        ...
        console.error('âŒ Eroare la obÈ›inerea produsului finit:', error);
        if (isMounted) {
          setFeedback({ type: 'error', message: 'Nu s-au putut Ã®ncÄƒrca detaliile produsului.' });
        }
      } finally {
        if (isMounted) {
          setLoading(false);
  ...
  const handleSubmit = useCallback(
    async (event: React.FormEvent<HTMLFormElement>) => {
      ...
      if (!selectedProductId) {
        setFeedback({ type: 'error', message: 'SelecteazÄƒ un produs din meniu.' });
        return;
      }

      if (formValues.min_stock < 0 || formValues.max_stock < 0 || formValues.current_stock < 0) {
        setFeedback({ type: 'error', message: 'Valorile trebuie sÄƒ fie pozitive.' });
        return;
      }

      if (formValues.max_stock < formValues.min_stock) {
        setFeedback({ type: 'error', message: 'Stocul maxim nu poate fi mai mic decÃ¢t cel minim.' });
        return;
      }
    ...
    <div className="finished-product-modal" role="dialog" aria-modal="true" aria-labelledby="finished-product-modal-title">
      ...
      <div className="finished-product-modal__content">
        <header className="finished-product-modal__header">
          <div>
            <h2 id="finished-product-modal-title">{isEdit ? 'EditeazÄƒ stoc produs finit' : 'AdaugÄƒ stoc produs finit'}</h2>
            {selectedProduct ? (
              <p>
        ...
        <form className="finished-product-modal__form" onSubmit={handleSubmit}>
          ...
          <footer className="finished-product-modal__footer">
            <button type="button" className="btn btn-ghost" onClick={handleClose} disabled={saving}>"AnuleazÄƒ"</button>
            <button type="submit" className="btn btn-primary" disabled={saving}>
              {saving ? 'Se salveazÄƒâ€¦' : isEdit ? 'SalveazÄƒ modificÄƒrile' : 'AdaugÄƒ stoc' }
            </button>
          </footer>
        </form>
      </div>
