    ...
    () => [
      ...
      },
      {
        ...
        valueFormatter: ({ value }) =>
          value === 'out' ? 'Epuizat' : value === 'critical' ? 'Critic' : value === 'low' ? 'ScÄƒzut' : 'OK',
      },
      {
        headerName: 'AcÈ›iuni',
        colId: 'actions',
        ...
        cellRenderer: () =>
          `<div class="stock-finished__row-actions">
            <button data-action="edit">Ã¢Å“ÂÃ¯Â¸Â Editare</button>
            <button data-action="recipe">ðŸ“‹ ReÈ›etÄƒ</button>
    ...
    (event: CellClickedEvent<FinishedProductStock>) => {
      ...
      if (action === 'edit') {
        ...
        setRecipeModalProduct({
          ...
          product_name: event.data.product_name ?? event.data.name ?? 'Produs fÄƒrÄƒ nume',
        ...
        if (confirm('Sigur doreÈ™ti sÄƒ È™tergi stocul configurat pentru acest produs finit?')) {
          httpClient
            ...
            .catch((error) => {
              console.error('Ã¢ÂÅ’ Eroare la È™tergerea produsului finit:', error);
              const message = error instanceof Error ? error.message : 'Nu s-a putut È™terge stocul produsului.';
      ...
      <div className="stock-finished__toolbar">
        ...
        <div className="stock-finished__toolbar-actions">
          <button type="button" className="btn btn-ghost" onClick={refreshData}>
            Ã¢Å¸Â³ ReÃ®mprospÄƒteazÄƒ
          </button>
          <button type="button" className="btn btn-primary" onClick={handleAdd}>
            Ã¢Å¾â€¢ AdaugÄƒ produs finit
          </button>
        </div>
      </div>
