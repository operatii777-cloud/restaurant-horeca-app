 ...
 * STANDARD: FoloseÈ™te ExcelPageLayout STANDARD (boogiT-like)
...
const StockCurrentCell: React.FC<StockCurrentCellProps> = ({ params, onAdjust }) => {
  if (!params.data) return <span>Ã¢â‚¬â€</span>;
...
//   const { t } = useTranslation();
  ...
  const ingredients = useMemo<IngredientStockItem[]>(() => {
    ...
    // Dar verificÄƒm dacÄƒ data este array sau obiect
    let result: Ingredient[] = [];
    if (Array.isArray(data)) {
      result = data;
    } else if (data && typeof data === 'object' && "DatÄƒ:" in data && Array.isArray((data as any).data)) {
      // Fallback: dacÄƒ data este { data: [...] }
      result = (data as any).data;
    } else if (data && typeof data === 'object' && 'ingredients' in data && Array.isArray((data as any).ingredients)) {
      // Fallback: dacÄƒ data este { ingredients: [...] }
  ...
  const columnDefs = useMemo<ColDef<IngredientStockItem>[]>(
    () => [
      ...
      },
      {
        ...
        field: "Status Stoc",
        width: 140,
        valueGetter: ({ data }: { data?: IngredientStockItem | null }) => data?.stock_status ?? 'Ã¢â‚¬â€',
        cellRenderer: ({ value }: ICellRendererParams<IngredientStockItem>) => {
          if (!value) return 'Ã¢â‚¬â€';
          const label = value === 'out' ? 'Epuizat' : value === 'low' ? 'ScÄƒzut' : value === 'critical' ? 'Critic' : 'OK';
        ...
        cellRenderer: (params: ICellRendererParams<IngredientStockItem>) => {
          ...
          const actions: GridAction[] = [
            ...
            },
            {
              ...
              variant: 'danger',
              onClick: () => {
                if (window.confirm(`EÈ™ti sigur cÄƒ vrei sÄƒ È™tergi ingredientul "${params.data!.name}"?`)) {
  ...
  if (decoratedIngredients.length === 0) {
    return (
      <EmptyState
        ...
        message="AdaugÄƒ primul ingredient pentru a Ã®ncepe."
