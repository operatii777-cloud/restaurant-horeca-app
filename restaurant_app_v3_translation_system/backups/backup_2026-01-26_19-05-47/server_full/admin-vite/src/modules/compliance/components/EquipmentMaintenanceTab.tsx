...
//   const { t } = useTranslation();
  ...
  const columnDefs = [
    ...
    { field: 'maintenance_type', headerName: 'Tip', width: 150, valueFormatter: (params: any) => {
      const types: any = {
        preventive: 'PreventivÄƒ',
        repair: 'ReparaÈ›ie',
        calibration: 'Calibrare',
      };
      return types[params.value] || params.value;
    }},
    { field: 'scheduled_date', headerName: 'Data ProgramatÄƒ', width: 180, valueFormatter: (params: any) => {
      if (!params.value) return '-';
      return new Date(params.value).toLocaleString('ro-RO');
    }},
    { field: 'completed_date', headerName: 'Data CompletatÄƒ', width: 180, valueFormatter: (params: any) => {
      if (!params.value) return '-';
      return new Date(params.value).toLocaleString('ro-RO');
    }},
    ...
    { field: 'result', headerName: 'Rezultat', width: 120, cellRenderer: (params: any) => {
      ...
      const labels: any = {
        ok: 'OK',
        needs_repair: 'NecesitÄƒ ReparaÈ›ie',
        replaced: 'ÃŽnlocuit',
      };
  ...
  const handleSaveMaintenance = async (formData: any) => {
    try {
      ...
      console.error('Eroare la salvarea mentenanÈ›ei:', error);
      alert('Eroare la salvarea mentenanÈ›ei');
    }
  };
