...
//   const { t } = useTranslation();
  ...
  const loadReport = async () => {
    ...
    try {
      ...
      setError(err.response?.data?.error || 'Eroare la Ã®ncÄƒrcarea raportului');
    } finally {
      setLoading(false);
    }
  };
  ...
  const exportToExcel = async (reportType: string) => {
    try {
      ...
      setError('Eroare la exportarea Ã®n Excel');
    }
  };
  ...
  return (
    <div className="sales-reports-page">
      ...
      {/* Filters */}
      <Card className="mb-4" style={{ margin: '8px 0' }}>
        <Card.Body>
          <Row>
            <Col md={3}>
            ...
            {activeTab === 'detailed' && (
              <Col md={3}>
                ...
                <Form.Select value={category} onChange={(e) => setCategory(e.target.value)}>
                  ...
                  <option value="bauturi">BÄƒuturi</option>
                </Form.Select>
              </Col>
            )}
            {activeTab === 'time-trends' && (
              <Col md={3}>
                <Form.Label>PerioadÄƒ</Form.Label>
        ...
        <Tab eventKey="detailed" title='ðŸ“‹ vanzari detaliate'>
          <Card>
            <Card.Header className="d-flex justify-content-between align-items-center">
              ...
              <Badge bg="info">{salesDetailed.length} Ã®nregistrÄƒri</Badge>
            </Card.Header>
            <Card.Body>
              {loading ? (
                ...
                <Alert variant="info">Nu existÄƒ date pentru perioada selectatÄƒ</Alert>
              ) : (
                  ...
                  <tbody>
                    {salesDetailed.map((item, index) => (
                      <tr key={`${item.order_id}-index`}>
                        ...
                        <td>{item.table_number || 'â€”'}</td>
                        <td>{item.client_identifier || 'â€”'}</td>
          ...
          <Card>
            ...
            <Card.Body>
              {loading ? (
                ...
                <>
                  {profitability.length > 0 && getProfitabilityChartData() && (
                    ...
                    <tbody>
                      {profitability.map((item) => (
                        <tr key={item.id}>
                          ...
                          <td>
                            <Badge bg={(item.profit_margin_percent ?? 0) >= 30 ? 'success' : (item.profit_margin_percent ?? 0) >= 20 ? 'warning' : 'danger'}>
          ...
          <Card>
            <Card.Header className="d-flex justify-content-between align-items-center">
              ...
              {customerBehaviorSummary && (
                <Badge bg="info">
                  {customerBehaviorSummary.totalCustomers} clienÈ›i analizaÈ›i
                </Badge>
              )}
            </Card.Header>
            <Card.Body>
              ...
              ) : !Array.isArray(customerBehavior) || customerBehavior.length === 0 ? (
                <Alert variant="info">Nu existÄƒ date pentru perioada selectatÄƒ</Alert>
              ) : (
                <>
                  {customerBehaviorSummary && (
                      ...
                      <Col md={4}>
                        <Card className="text-center">
                          <Card.Body>
                            ...
                            <h4>{(customerBehaviorSummary.avgOrdersPerCustomer ?? 0).toFixed(1)}</h4>
                          </Card.Body>
                        </Card>
                      </Col>
                      <Col md={4}>
                        ...
                        <Card className="text-center">
                          <Card.Body>
                            ...
                            <h6 className="text-muted">
                              {customerBehaviorSummary.topSpender?.client_identifier || 'â€”'}
                            </h6>
                            <p className="mb-0">
                  ...
                  <Table striped hover responsive>
                    ...
                    <tbody>
                      {Array.isArray(customerBehavior) && customerBehavior.map((item, index) => (
          ...
          <Card>
            ...
            <Card.Body>
              {loading ? (
                ...
                <>
                  {timeTrends.length > 0 && getTimeTrendsChartData() && (
