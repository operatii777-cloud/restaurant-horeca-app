    ...
    <div className="client-monitor">
      ...
      <div className="client-monitor__content">
        ...
        <div className="client-monitor__column client-monitor__column--progress">
          ...
          <div className="client-monitor__orders">
            {inProgress.length === 0 ? (
              <div className="client-monitor__empty">
                <span>Nicio comandă în preparare</span>
              </div>
            ) : (
              inProgress.slice(0, 12).map((order, idx) => (
                <div key={order.id} className="client-monitor__card client-monitor__card--progress">
                  <div className="client-monitor__card-number">
                    {order.orderNumber}
                  </div>
                  <div className="client-monitor__card-details">
                    {order.type === 'delivery' ? '🛵' : order.type === 'takeaway' ? '🥡' : `M${order.tableNumber}`}
                    <span className="client-monitor__card-time">
                      {getWaitTime(order.timestamp)} min
                    </span>
                  </div>
                  <div className="client-monitor__card-stations">
                    {order.kitchenPending > 0 && (
                      <span className="client-monitor__station client-monitor__station--pending">
                        <ChefHat size={14} /> {order.kitchenPending}
                      </span>
                    )}
                    {order.kitchenDone > 0 && (
                      <span className="client-monitor__station client-monitor__station--done">
                        <ChefHat size={14} /> ✓{order.kitchenDone}
                      </span>
                    )}
                    {order.barPending > 0 && (
                      <span className="client-monitor__station client-monitor__station--pending">
                        <Wine size={14} /> {order.barPending}
                      </span>
                    )}
                    {order.barDone > 0 && (
                      <span className="client-monitor__station client-monitor__station--done">
                        <Wine size={14} /> ✓{order.barDone}
                      </span>
                    )}
                  </div>
                </div>
            ...
            {inProgress.length > 12 && (
              <div className="client-monitor__more">
                ... și alte {inProgress.length - 12} comenzi
              </div>
            )}
          </div>
        </div>
        ...
        <div className="client-monitor__column client-monitor__column--ready">
          ...
          <div className="client-monitor__orders client-monitor__orders--ready">
            {ready.length === 0 ? (
              ...
              ready.slice(0, 8).map((order, idx) => {
                const isFlashing = flashingOrders.includes(order.orderNumber);
                return (
                  <div 
                    key={order.id} 
                    className={`client-monitor__card client-monitor__card--ready ${isFlashing ? 'client-monitor__card--flash' : ''}`}
                  >
                    <div className="client-monitor__card-number client-monitor__card-number--large">
                      {order.orderNumber}
                    </div>
                    <div className="client-monitor__card-details">
                      {order.type === 'delivery' ? '🛵 Delivery' : order.type === 'takeaway' ? '🥡 Takeaway' : `🍽️ Masa ${order.tableNumber}`}
                    </div>
                  </div>
                );
              })
