...
//   const { t } = useTranslation();
  ...
  const fetchOrders = async () => {
    try {
      ...
      if (response.data && response.data.data) {
        // FiltreazÄƒ doar comenzi drive-thru
        const driveThruOrders = response.data.data.filter(
          (o: any) => o.order_source === 'DRIVE_THRU' || o.type === "Drive-Thru" || o.type === 'drive-thru'
        );
  ...
  const stats = useMemo(() => {
    ...
    // CalculeazÄƒ timp mediu (dacÄƒ existÄƒ date)
    ...
    <div className="drivethru-orders-page">
      ...
      <Row className="mb-4">
        ...
        <Col md={3}>
          <StatCard
            ...
            helper={`${stats.ordersWithTime} comenzi mÄƒsurate`}
            icon={<span>â±ï¸</span>}
          />
        </Col>
        <Col md={3}>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>Detalii ComandÄƒ Drive-Thru #{selectedOrder?.id}</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          {selectedOrder && (
            <>
              <Row className="mb-3">
                ...
                <Col md={6}>
                  <strong>"BandÄƒ:"</strong> <Badge bg="warning">{selectedOrder.lane_number || 'N/A'}</Badge>
                </Col>
              </Row>
              ...
              {selectedOrder.fiscal_receipt_printed && (
                <p><strong>Bon fiscal:</strong> âœ… Printat</p>
              )}
            </>
          )}
        </Modal.Body>
